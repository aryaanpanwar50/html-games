...
var fieldGameOver, fieldDistance, fieldHighScore;
var highScore = parseInt(localStorage.getItem('rabbitGameHighScore')) || 0;
...
Hero = function() {
  ...
  earGeom = new THREE.BufferGeometry();
  earGeom.setAttribute('position', new THREE.BufferAttribute(positions, 3));
  earGeom.setIndex(new THREE.BufferAttribute(indices, 1));
...
Monster = function(){
  ...
  var toothGeom = new THREE.BufferGeometry();
  const toothVertices = new Float32Array([
    // Front face
    -1.0, -1.0, 0.5,  // 0
     0.0, -1.0, 0.5,  // 1
    -1.0,  1.0, 0.5,  // 2
  ...
  const toothIndices = new Uint16Array([
    0, 1, 2,    1, 3, 2,  // front
    5, 4, 7,    4, 6, 7,  // back
    4, 0, 6,    0, 2, 6,  // left
    1, 5, 3,    5, 7, 3,  // right
  ...
  toothGeom.setAttribute('position', new THREE.BufferAttribute(toothVertices, 3));
  toothGeom.setIndex(new THREE.BufferAttribute(toothIndices, 1));
  ...
  var earGeom = new THREE.BufferGeometry();
  const earVertices = new Float32Array([
    // Front
    -4, 0, 1,    // 0
    4, 0, 1,     // 1
    -4, 6, 1,    // 2
  ...
  const earIndices = new Uint16Array([
    0, 1, 2,    1, 3, 2,  // front
    5, 4, 7,    4, 6, 7,  // back
    4, 0, 6,    0, 2, 6,  // left
    1, 5, 3,    5, 7, 3,  // right
  ...
  earGeom.setAttribute('position', new THREE.BufferAttribute(earVertices, 3));
  earGeom.setIndex(new THREE.BufferAttribute(earIndices, 1));
  earGeom.computeVertexNormals();
...
Carrot = function() {
  ...
  // Create carrot body using BufferGeometry
  var bodyGeom = new THREE.CylinderBufferGeometry(5, 3, 10, 4, 1);
  // Modify the top vertices to create carrot shape
  const positions = bodyGeom.getAttribute('position');
  for (let i = 0; i < positions.count; i++) {
    if (i >= positions.count - 8) { // Top vertices
      const y = positions.getY(i);
      positions.setY(i, y + (i % 2 ? -3 : 2)); // Alternate vertices up/down
    }
  }
  ...
  // Create leaf using BufferGeometry
  ...
  const leafIndices = new Uint16Array([
    0, 1, 2,    1, 3, 2,  // front
    5, 4, 7,    4, 6, 7,  // back
    4, 0, 6,    0, 2, 6,  // left
    1, 5, 3,    5, 7, 3,  // right
  ...
  leafGeom.setAttribute('position', new THREE.BufferAttribute(leafVerts, 3));
  leafGeom.setIndex(new THREE.BufferAttribute(leafIndices, 1));
...
function updateDistance(){
  ...
  fieldDistance.innerHTML = d;
  
  // Update high score if current distance is higher
  if (d > highScore) {
    highScore = d;
    localStorage.setItem('rabbitGameHighScore', highScore);
  }
...
Trunc = function(){
  // Initialize mesh property first
  this.mesh = new THREE.Group();
  ...
  // Create cylinder using modern BufferGeometry
  var geom = new THREE.CylinderBufferGeometry(topRadius, bottomRadius, truncHeight, nhSegments, nvSegments);
  geom.applyMatrix4(new THREE.Matrix4().makeTranslation(0,truncHeight/2,0));
  
  // Create base trunk mesh
  var trunkMesh = new THREE.Mesh(geom, matTrunc);
  this.mesh.add(trunkMesh);

  // Get vertex positions from buffer
  var positions = geom.getAttribute('position');
  
  // Add decorative elements based on vertex positions
  for(let i = 0; i < positions.count; i++) {
    ...
    if(Math.random() > 0.5 && y > 10 && y < truncHeight - 10) {
      ...
      var branchGeometry = new THREE.CylinderBufferGeometry(thickness/2, thickness, h, 3, 1);
      ...
      var vec = new THREE.Vector3(x, 2, z);
      var axis = new THREE.Vector3(0,1,0);
