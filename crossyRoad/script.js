...
function reset() {
  ...
  scoreElement.innerHTML = `Press UP<br>High Score: ${highScore}`;
...
function startGame() {
  if (ready) {
    ...
    setTimeout(spawnPowerup, POWERUP_SPAWN_INTERVAL);
  }
}
...
function createPowerup() {
  const powerupGeometry = new THREE.BoxBufferGeometry(20, 20, 20);
...
function spawnPowerup() {
  ...
  powerup.position.x = Math.cos(powerupAngle) * trackRadius - arcCenterX;
  powerup.position.y = Math.sin(powerupAngle) * trackRadius;
  powerup.userData.angle = powerupAngle;
  scene.add(powerup);
}

function checkPowerupCollection() {
  if (!powerup || isInvincible) return;
  ...
  if (getDistance(playerPosition, powerupPosition) < 40) {
    collectPowerup();
  }
}
...
function collectPowerup() {
  ...
  // Change player car color to indicate invincibility
  playerCar.children[0].material.color.setHex(0xffd700);
  ...
  invincibilityTimer = setTimeout(() => {
    isInvincible = false;
    playerCar.children[0].material.color.setHex(pickRandom(vehicleColors));
    setTimeout(spawnPowerup, POWERUP_SPAWN_INTERVAL);
